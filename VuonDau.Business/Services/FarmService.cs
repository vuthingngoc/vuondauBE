            ...
            if (request.Status == null)
            {
                ...
                else
                {
                    if (request.FarmerId == null)
                    {
                        ...
                        {
                            return await Get(f => f.Name.ToLower().Contains(request.Name.ToLower()) && f.FarmTypeId == request.FarmTypeId
                                    && f.AreaId == request.AreaId).OrderBy(f => f.Name).OrderByDescending(f => f.Status).ProjectTo<FarmViewModel>(_mapper).ToListAsync();
                        }
                    }
                    else
                    {
                        ...
                        {
                            return await Get(f => f.Name.ToLower().Contains(request.Name.ToLower()) && f.FarmTypeId == request.FarmTypeId
                                    && f.FarmerId == request.FarmerId).OrderBy(f => f.Name).OrderByDescending(f => f.Status).ProjectTo<FarmViewModel>(_mapper).ToListAsync();
                        }
                        else
                        {
                            return await Get(f => f.Name.ToLower().Contains(request.Name.ToLower()) && f.FarmTypeId == request.FarmTypeId
